 ...
 * 遍历集合元素的目的
 *
 * java.util.Iterator接口,是迭代器接口,规定了遍历集合的的相关操作
 * 不同的集合实现了一个可以遍历其元素的迭代器实现类,我们无须知道这些实现类的名字,只当
 * 他们是Iterator看待即可.
...
public class IteratorDemo {
    public static void main(String[] args) {
        ...
        /*
            boolean hasNext()
            通过迭代器判断集合是否还有下一个元素可以获取(第一次判断就是判断集合
            是否有第一个元素,以此类推.)
         */
        while(it.hasNext()){
            /*
...
//                c.remove(str);
                /*
                   void remove()
                   迭代器通过remove方法是集合中删除本次通过next方法获取的元素
                 */
                it.remove();
            }
        }
